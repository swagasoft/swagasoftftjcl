...
searchModel = { 
  outletCode: '',day:null, month: null, year : null
  };
  ngOnInit() {
    this.getAllOutlets();
  } 
...
this.searchByMonth();
  ...
  onDateSelect(event){
    console.log(event);
    this.searchModel.day = event.day;
    this.searchModel.month = event.month;
    this.searchModel.year = event.year;
  ...
  searchByDay(){
    ...
    this.total_slg = 0;
      ...
      this.slg_return = 0;
    this.loading = true;
    this.distService.getOutletSuppliesByDay(this.searchModel).subscribe(
      res => {
        this.loading = false;
        this.selectedOutlet = res['record'];
        console.log(this.selectedOutlet);
